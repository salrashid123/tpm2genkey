...
* imports an existing RSA|ECC|HMAC|AES key material into a TPM
...
| **`-rsaScheme`** | RSA Scheme (`rsassa|rsapss (default rsassa)`) |
| **`-hashScheme`** | HashScheme to use (`sha256|sha384|sha512 (default sha256)`) |
...
### Import external RSA|ECC|AES|HMAC Key

The following uses a softwareTPM as the target (`--tpm-path="127.0.0.1:2321"`) to import various key types. To use a real tpm, omit this
...
tpm2_flushcontext -t
tpm2_load -C /tmp/primary.ctx -u /tmp/key.pub -r /tmp/key.prv -c /tmp/key.ctx 

echo -n "foo" > /tmp/data.txt 
tpm2_sign -c /tmp/key.ctx -g sha256 -o /tmp/sig.rssa -f plain /tmp/data.txt
```
...
echo -n "foo" > /tmp/data.txt 
tpm2_sign -c /tmp/key.ctx -g sha256 -p bar -o /tmp/sig.rssa -f plain /tmp/data.txt
```
...
tpm2_policyauthvalue -S /tmp/session.dat -L /tmp/policy.dat

tpm2_sign -c /tmp/key.ctx -g sha256 -p bar -o /tmp/sig.rssa -f plain /tmp/data.txt -p"session:/tmp/session.dat+bar"
```
...
tpm2_flushcontext -t && tpm2_flushcontext -s && tpm2_flushcontext -l
tpm2_startauthsession --policy-session -S /tmp/session.dat
tpm2_pcrread sha256:23 -o /tmp/pcr23_val.bin
tpm2_policypcr -S /tmp/session.dat -l "sha256:23"   -L /tmp/policy.dat -f /tmp/pcr23_val.bin

echo -n "foo" > /tmp/data.txt 
tpm2_sign -c /tmp/key.ctx -g sha256 -o /tmp/sig.rssa -f plain /tmp/data.txt -p"session:/tmp/session.dat"
```
...
echo -n "foo" > /tmp/data.txt 
tpm2_sign -c /tmp/key.ctx -g sha256 -o /tmp/sig.ecc -f plain /tmp/data.txt
```
...
echo "foo" > /tmp/secret.dat
openssl rand  -out /tmp/iv.bin 16

tpm2_load -C /tmp/primary.ctx -u /tmp/key.pub -r /tmp/key.prv -n /tmp/key.name -c /tmp/aes.ctx
tpm2_flushcontext -t
tpm2_encryptdecrypt -Q --iv /tmp/iv.bin -c /tmp/aes.ctx -o /tmp/cipher.out /tmp/secret.dat
tpm2_encryptdecrypt -Q --iv /tmp/iv.bin -c /tmp/aes.ctx -d -o /tmp/plain.out /tmp/cipher.out
```
...
tpm2_flushcontext -t
tpm2_load -C /tmp/primary.ctx -u /tmp/key.pub -r /tmp/key.prv -n /tmp/key.name -c /tmp/hmac.ctx

echo -n "foo" | tpm2_hmac -g sha256 -c /tmp/hmac.ctx | xxd -p -c 256
```
